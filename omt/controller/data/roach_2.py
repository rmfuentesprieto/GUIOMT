            ...
            for names in bram['bram_names']:
                ...
                if len(real_name) > 0:
                    real_array = struct.unpack('>'+str(array_size) +
                                               data_type, self.fpga.read(real_name,array_size*data_type_dictionart[data_type], 0))
                ...
                if len(imag_name) > 0:
                    imag_array = struct.unpack('>'+str(array_size) +
                                               data_type, self.fpga.read(imag_name,array_size*data_type_dictionart[data_type], 0))
                    imag_data.append(imag_array)
                else:
                    have_imag = have_imag and False

            final_array = numpy.zeros((1,array_size*nbram))

            for cont0 in range(array_size):
